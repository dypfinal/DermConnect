...
class Doctor(models.Model):  # doctor details
    ...
    )  # doctor department from choices given as list
...
class Patient(models.Model):  # patient details
    ...
    image = models.ImageField(
        default="default.png", upload_to="profile_pics", null=True, blank=True
    )  # profile picture
    firstname = models.CharField(
        max_length=100, default="firstname"
    )  # patient firstname
...
class Appointment(models.Model):  # patient appointment details
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="PatientApp"
    )  # patient foreign key
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, related_name="DoctorApp"
    )  # doctor foreign key
    description = models.TextField(
        max_length=500
    )  # description (symptoms/how to take medicines/causes/precautions)
...
class PatHealth(models.Model):  # personal health details of patient
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="PatientHealth"
    )  # patient foreign key
    ...
    diseases = models.CharField(
        max_length=2000, default="somediseases"
    )  # patient diseases
    medicines = models.CharField(
        max_length=2000, default="somemedicines"
    )  # patient medicines
    ts = models.CharField(
        max_length=2000, default="treatments/surgery"
    )  # patient treatments/surgeries
    status = models.BooleanField(
        default=False
    )  # field to determine whether this instance is filled by the patient or not. if it is not built, it is False and patient has to first fill it
...
class PatAdmit(models.Model):  # patient admit details
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="PatientAdmit"
    )  # patient foreign key
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, related_name="DoctorAdmit"
    )  # doctor foreign key
    ...
    )  # date on which patient is discharged
...
class Charges(models.Model):  # charges with respect to admit
    Admitinfo = models.ForeignKey(
        PatAdmit, on_delete=models.CASCADE, related_name="AdmitDetails"
    )  # admit foreign key
    commodity = models.ForeignKey(
        Medicines, on_delete=models.CASCADE, related_name="AdmitDetails"
...
class ChargesApt(models.Model):  # charges with respect to appointment
    Aptinfo = models.ForeignKey(
        Appointment, on_delete=models.CASCADE, related_name="AptDetails"
    )  # appointment foreign key
    commodity = models.ForeignKey(
        Medicines, on_delete=models.CASCADE, related_name="AptDetails"
    )  # medicine foreign key
    quantity = models.IntegerField(default=1)  # quantity of medicine(commodity)
...
class DoctorProfessional(models.Model):  # professional details of a doctor
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, related_name="DoctorProfessional"
    )  # doctor foreign key
...
class OperationCosts(
    models.Model
):  # for easier and smoother day-to-day hospital works, admin can change costs
...
class CovidVaccination(models.Model):  # to track and record patients who are vaccinated
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="PatientVaccination"
    )  # patient foreign key
    vaccine = models.ForeignKey(
        Medicines, on_delete=models.CASCADE, related_name="Vaccine"
    )  # medicine(here, vaccine) foreign key
