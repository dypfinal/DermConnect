...
class ContactusForm(
    forms.Form
):  # contact us form (feedback), used by patients/doctors to send feedbacks using mail to admins
    Name = forms.CharField(
        max_length=30, label="", widget=forms.TextInput(attrs={"placeholder": "NAME"})
    )
...
class DoctorRegisterForm(UserCreationForm):  # used to register a doctor
    ...
    firstname = forms.CharField(
        label="", widget=forms.TextInput(attrs={"placeholder": "FIRSTNAME"})
    )
    firstname.widget.attrs.update({"class": "app-form-control"})

    lastname = forms.CharField(
        label="", widget=forms.TextInput(attrs={"placeholder": "LASTNAME"})
    )
    lastname.widget.attrs.update({"class": "app-form-control"})

    dob = forms.DateField(label="", widget=SelectDateWidget(years=range(1950, 2000)))
    dob.widget.attrs.update({"class": "app-form-control-date"})

    address = forms.CharField(
        label="", widget=forms.TextInput(attrs={"placeholder": "ADDRESS"})
    )
    address.widget.attrs.update({"class": "app-form-control"})

    city = forms.CharField(
        label="", widget=forms.TextInput(attrs={"placeholder": "CITY"})
    )
    city.widget.attrs.update({"class": "app-form-control"})

    country = forms.CharField(
        label="", widget=forms.TextInput(attrs={"placeholder": "COUNTRY"})
    )
    country.widget.attrs.update({"class": "app-form-control"})

    postalcode = forms.IntegerField(
        label="", widget=forms.TextInput(attrs={"placeholder": "POSTAL CODE"})
    )
    ...
    password2 = forms.CharField(
        label="", widget=forms.PasswordInput(attrs={"placeholder": "RE-CONFIRM"})
    )
...
class DoctorUpdateForm(forms.ModelForm):  # used to edit a doctor instance
    ...
    dob = forms.DateField(widget=SelectDateWidget(years=range(1940, 2000)))
    address = forms.CharField()
    city = forms.CharField()
    country = forms.CharField()
    postalcode = forms.IntegerField()
...
class AdminRegisterForm(UserCreationForm):  # used to register an admin
    ...
    dob = forms.DateField(label="", widget=SelectDateWidget(years=range(1940, 2024)))
    dob.widget.attrs.update({"class": "app-form-control-date"})
...
class AdminUpdateForm(forms.ModelForm):  # used to edit an admin instance
    firstname = forms.CharField()
    lastname = forms.CharField()
    dob = forms.DateField(widget=SelectDateWidget(years=range(1940, 2024)))
    address = forms.CharField()
    city = forms.CharField()
    country = forms.CharField()
    postalcode = forms.IntegerField()
...
class PatientAppointmentForm(
    forms.ModelForm
):  # used to register an appointment by patient
    doctor = forms.TypedChoiceField(
        label=""
    )  # doctor is chosed from existing doctors in hospital database
    doctor.widget.attrs.update({"class": "app-form-control"})
    # doctorId=forms.CharField(widget=forms.Select(choices=c))
    calldate = forms.DateField(
        label="", widget=SelectDateWidget(years=range(2021, 2024))
    )  # date of appointment
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["doctor"].choices = [
            (c.id, c.firstname + "(" + c.department + ")")
            for c in Doctor.objects.filter(status=True).all()
        ]  # list of doctors to choose from, taken fresh from database
        self.fields["calltime"].choices = [
            ("9:00 AM", "9:00 AM"),
            ("9:15 AM", "9:15 AM"),
            ("9:30 AM", "9:30 AM"),
            ("9:45 AM", "9:45 AM"),
...
class AdminAppointmentForm(forms.ModelForm):  # used to register an appointment by admin
    ...
    patient = forms.TypedChoiceField(
        label=""
    )  # patient is chosed from existing doctors in hospital database
    patient.widget.attrs.update({"class": "app-form-control"})
    calldate = forms.DateField(
        label="", widget=SelectDateWidget(years=range(2024, 2024))
    )  # date of appointment
    calldate.widget.attrs.update({"class": "app-form-control-date"})
    calltime = forms.TypedChoiceField(label="")  # time of appointment
    calltime.widget.attrs.update({"class": "app-form-control"})
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["patient"].choices = [
            (c.id, c.firstname) for c in Patient.objects.filter(status=True).all()
        ]  # list of patients to choose from, taken fresh from database
        self.fields["calltime"].choices = [
            ("9:00 AM", "9:00 AM"),
            ("9:15 AM", "9:15 AM"),
            ("9:30 AM", "9:30 AM"),
...
class AppointmentEditForm(
    forms.ModelForm
):  # doctor can edit appointment description field, be it adding new lines or deleting a few of the old one
    description = forms.CharField(
        max_length=300,
        label="",
        widget=forms.TextInput(attrs={"placeholder": "DESCRIPTION"}),
...
class DoctorProfessionalUpdateForm(forms.ModelForm):  # doctor can edit their feees
    appfees = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "APPOINTMENT FEES"})
    )
    appfees.widget.attrs.update({"class": "app-form-control"})
    admfees = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "ADMIT FEES"})
    )
...
class AddMedForm(forms.ModelForm):  # admin can add medicines to database
    ...
    price = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "PRICE"})
    )
...
class OpcostsForm(
    forms.Form
):  # admin can change hospital operation charges, like maintenence fee
    maintenance = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "MAINTAINANCE CHARGE"})
    )
    maintenance.widget.attrs.update({"class": "app-form-control"})
    hospfee = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "HOSPITAL FEE"})
    )
    hospfee.widget.attrs.update({"class": "app-form-control"})
    roomfee = forms.FloatField(
        label="", widget=forms.TextInput(attrs={"placeholder": "ROOM FEE"})
    )
    roomfee.widget.attrs.update({"class": "app-form-control"})


class CovidVaccinationApplicationForm(
    forms.Form
):  # patient can apply for covid vaccine
    ...
    def __init__(self, *args, **kwargs):
        ...
        covaxin = (
            Medicines.objects.all().filter(name="covaxin").first()
        )  # fetch covaxin vaccine information from database
        covishield = (
            Medicines.objects.all().filter(name="covishield").first()
        )  # fetch covishield vaccine information from database
        self.fields["commodity"].choices = [
            (covaxin.id, covaxin.name),
            (covishield.id, covishield.name),
        ]
